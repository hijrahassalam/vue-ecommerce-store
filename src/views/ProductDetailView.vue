<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div v-else-if="productStore.currentProduct" class="grid grid-cols-1 md:grid-cols-2 gap-12">
      <img :src="product.image_url" :alt="product.name" class="w-full rounded-2xl shadow-lg" />
      <div>
        ...
        <div class="flex items-center space-x-4">
          <div class="flex items-center border border-gray-300 rounded-lg">
            <button @click="decrementQty" class="px-4 py-3 text-gray-600 hover:text-gray-900"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg></button>
            <span class="px-4 py-3 font-medium">{{ quantity }}</span>
            <button @click="incrementQty" class="px-4 py-3 text-gray-600 hover:text-gray-900"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg></button>
          </div>
          <button @click="addToCart" :disabled="product.stock === 0 || cartStore.loading" class="btn-primary flex-1 disabled:opacity-50 disabled:cursor-not-allowed">
